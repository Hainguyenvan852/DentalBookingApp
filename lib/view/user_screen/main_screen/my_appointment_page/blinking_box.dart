...
class AppointmentStateBox extends StatelessWidget {
  ...
  String getStatus(){
    final status = switch(appointment.status){
      ...
      'no_show' => 'Bỏ hẹn',
      'completed' => 'Kết thúc',
      'canceled_by_patient' => 'Đã hủy',
      'canceled_by_clinic' => 'Đã hủy',
      _ => 'Chưa duyệt'
    };
  ...
  Color getColorWithState(){
    final color = switch(appointment.status){
      ...
      'canceled_by_patient' => Colors.grey,
      'canceled_by_clinic' => Colors.grey,
      _ => Colors.red,
    };
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Center(child: Text(getStatus(), style: TextStyle(color: Colors.white, fontSize: 12),)),
    );
  }
}