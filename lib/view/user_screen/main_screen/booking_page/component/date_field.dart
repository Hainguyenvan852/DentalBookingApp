import 'package:dental_booking_app/logic/slot_cubit.dart';
...
class _DateFieldState extends State<DateField> {
  ...
  void initState() {
    super.initState();
    dateCtrl.text = DateFormat('EE, dd/MM/yyyy', 'vi_VN').format(DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day));
  }

  @override
  Widget build(BuildContext context) {
    final dateCubit = context.read<DateCubit>();
    final slotCubit = context.read<TimeSlotCubit>();
                ...
                child: Padding(
                  ...
                  child: TextFormField(
                    ...
                    onTap: () async {
                      ...
                      if (result != null){
                        final selectedDate = DateTime(result.year, result.month, result.day, DateTime.now().hour, DateTime.now().minute, DateTime.now().second);
                        dateCtrl.text = DateFormat('EE, dd/MM/yyyy', 'vi_VN').format(selectedDate);
                        slotCubit.select(null);
