                      ...
                      child: GridView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final color = switch(status){
                            ...
                            SlotStatus.overtime => Colors.grey,
...
class TimeBox extends StatelessWidget {
  ...
  const TimeBox({super.key, required this.slot, required this.selected, required this.onTap, required this.color, required this.status});
  ...
  String getStatus(SlotStatus s){
    final status = switch(s){
      ...
      SlotStatus.overtime => 'Đã quá giờ',
      _ => 'Vắng',
    };
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => (status == SlotStatus.userBooked || status == SlotStatus.overtime) ? null : onTap(),
      child: Container(
        decoration: BoxDecoration(
          border: Border.all(color: Colors.black),
          borderRadius: BorderRadius.circular(5),
          color: colorByState(selected, status!)
        ),
        child: Column(
...
Color colorByState(bool selected, SlotStatus status){
  if(selected){
    return Colors.greenAccent;
  } else if(status == SlotStatus.closed || status == SlotStatus.overtime){
