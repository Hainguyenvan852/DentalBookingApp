...
class _MedicalRecordScreenState extends State<MedicalRecordScreen> {
  ...
  Widget build(BuildContext context) {
    return RepositoryProvider(
      create: (context) => repo,
      child: BlocProvider(
          create: (context) => GalleryCubit(repo: repo, userId: widget.patientId),
          child: Scaffold(
    ...
    return BlocBuilder<GalleryCubit, GalleryState>(
        builder: (context, state) {
          ...
          if(state.items.isEmpty){
            return Center(child: Text('Bệnh nhân chưa có bệnh án', ),);
          }
          final photos = state.items.where((photo) => photo.type == 'medical_records');

          final grouped = groupBy(photos, (img) => DateFormat('dd/MM/yyy').format(img.createdAt));
